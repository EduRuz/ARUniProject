...
LevelSelectState::LevelSelectState(gef::Platform& platform) :
	State(platform)
{
}


void LevelSelectState::init(gef::AudioManager* audioManager_, gef::InputManager* inputManager_, gef::SpriteRenderer* spriteRenderer_, class gef::Renderer3D* renderer3d_, bool vita)
{
	...
	backgroundIcon = CreateTextureFromPNG("RPG Ambient Background.png", platform_);
...
void LevelSelectState::update(float fps) {
...
void LevelSelectState::render(float fps, gef::Font * font_)
{

	spriteRenderer->Begin();

	gef::Sprite background2;
	background2.set_texture(backgroundIcon);
	background2.set_position(gef::Vector4(platform_.width()*0.5f, platform_.height()*0.5f, -0.99f));
	background2.set_height(544.0f);
	background2.set_width(960.0f);
